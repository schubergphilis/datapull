...
exports.run = function (pipeline, dryRun) {
  ...
  return pipeline.origin.runner(config)
    ...
    .then(transformedData => {
      // build message:
      const message = buildMessage(pipeline.messageTemplate, pipeline, transformedData);

      // run destination
      console.log('[Pipeline] base64 record size', new Buffer(JSON.stringify(message)).toString('base64').length);

      if (!pipeline.destination.runner) {
        return console.warn('[Pipeline] destination does not specify a runner');
      }

      return pipeline.destination.runner([message], dryRun);
    })
    .catch(err => {
