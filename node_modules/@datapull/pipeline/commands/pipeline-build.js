  ...
  Object.keys(config)
    ...
    .forEach(k => {
      ...
      if (runnerModule.isConfiguration) {
        pipelineSteps.configurations.push({k, runner: new runnerModule.constructor(config[k]), config: config[k]})
      }

      if (runnerModule.isTransformer) {
        pipelineSteps.transformers.push({k, runner: new runnerModule.constructor(config[k]), config: config[k]})
      }

      if (runnerModule.isOrigin) {
        pipelineSteps.origins.push({k, runner: new runnerModule.constructor(config[k]), config: config[k]})
      }

      if (runnerModule.isDestination) {
        pipelineSteps.destinations.push({k, runner: new runnerModule.constructor(config[k]), config: config[k]})
      }
    });
