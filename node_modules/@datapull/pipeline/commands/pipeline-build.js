...
exports.build = function (config) {
  ...
  Object.keys(config)
    ...
    .forEach(k => {
      ...
      if (runnerModule.isConfiguration) {
        pipelineSteps.configurations.push({k, runner: new runnerModule.constructor(pureConfig), config: config[k]})
      }

      if (runnerModule.isTransformer) {
        pipelineSteps.transformers.push({k, runner: new runnerModule.constructor(pureConfig), config: config[k]})
      }

      if (runnerModule.isOrigin) {
        pipelineSteps.origins.push({k, runner: new runnerModule.constructor(pureConfig), config: config[k]})
      }

      if (runnerModule.isDestination) {
        pipelineSteps.destinations.push({k, runner: new runnerModule.constructor(pureConfig), config: config[k]})
      }
    });
  ...
  console.debug('[Pipeline] Intermediate pipelineSteps.configurations', pipelineSteps.configurations.length);
  console.debug('[Pipeline] Intermediate pipelineSteps.origins', pipelineSteps.origins.length);
  ...
  pipelineSteps.configurations.forEach(configurator => {
    ...
    pipelines.forEach(p => {
      if (p.extendMessage) {
        ...
        p.messageTemplate = Object.assign({}, p.messageTemplate, p.extendMessage);
      }
    });
  ...
  pipelineSteps.origins.forEach(origin => {
    ...
    pipelines.forEach(p => {
      ...
      if (origin.config.transform) {
        ...
        origin.config.transform.forEach(transformer => {
          Object.keys(transformer).forEach(transformerName => {
            ...
            originDeclaration.transformers.push({name: transformerName, config: transformerConfig});
          });
        });
      }
      ...
      if (p.origin) {
        newPipelines.push(Object.assign(
          deepcopy(p),
          {origin: Object.assign({}, originDeclaration, {extendMessage: origin.config.extendMessage})})
        );
      } else {
        p.origin = Object.assign({}, originDeclaration, {extendMessage: origin.config.extendMessage});
        newPipelines.push(p);
      }
    });
  });
  ...
  pipelines.forEach(p => {
    ...
    if (p.origin.extendMessage) {
      p.messageTemplate = Object.assign({}, p.messageTemplate, p.origin.extendMessage);
    }
  });
  ...
  pipelines.forEach(p => {
    if (p.origin && p.origin.transformers) {
      p.origin.transformers.forEach(t => {
        ...
        const transformerModuleName = `@datapull/${t.name}-transformer`;
        ...
        p.transformers.push(Object.assign({}, t, {runner: new transformerModule.constructor(t.config)}));
      })
    }
  });
  ...
  pipelineSteps.destinations.forEach(destination => {
    ...
    if (destination.config.extendMessage) {
      pipelines.forEach(p => {
        ...
        p.messageTemplate = Object.assign({}, p.messageTemplate, destination.config.extendMessage);
      });
    }
  });
