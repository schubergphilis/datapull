  ...
  constructor(config) {
    ...
    ['user', 'host', 'database', 'password', 'port'].forEach(param => {
      if (!this.config[param]) {
        throw Error(
          `[Redshift destination] Parameter "${param}" is required for this destination`
        );
      }
    });
  ...
  async insertNewRows(messages) {
    ...
    for (let m of messages) {
      ...
      if (this.config.primaryKey && m[this.config.primaryKey]) {
        ...
        const resp = await client.query(checkExistingRowQuery, [
      ...
      const insertQuery = `INSERT INTO ${this.config.table} (${keys.join(
        ','
