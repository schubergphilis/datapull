  ...
  program
    ...
    .action(function (file) {
      ...
      pipelines.forEach(p => {
        ...
        table.push([JSON.stringify(configValues[0]),
          {rowSpan, content: p.origin.name},
          {rowSpan, content: p.transformers.map(t => `${t.name}(${t.config})`).join('\n')},
        ...
        console.log(datapullPipeline.buildMessage(p.messageTemplate, {}, 'sample data'));
      });
