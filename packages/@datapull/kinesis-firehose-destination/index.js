...
class KinesisFirehoseDestination {
  constructor(config) {
    ...
    this.serviceClient = new aws.Firehose(pick(this.config, ['region', 'accessKeyId', 'secretAccessKey']));
  }
  ...
  sendMessages(messages, dryRun) {
    ...
    console.log('[Firehose Destination] ' + messages.length + ' message(s) to send');
    ...
    return new Promise((resolve, reject) => {
      ...
      if (dryRun) {
        console.log("[Firehose Destination] Dry run: skipping sending messages.");
      ...
      const sendToFirehose = (records) => {
        return new Promise((resolve, reject) => {
          console.log(`[Firehose Destination] pushing ${params.Records.length} messages to aws (out of ${messages.length} total)`);
          ...
          const sendAttempt = (records) => {
            ...
            console.log(`[Firehose Destination] attempting to send ${records.length} messages. Collection size: ${collectionSize}`);

            this.serviceClient['putRecordBatch'].call(this.serviceClient, params, (err, resp) => {
              if (err) {
              ...
              if (resp.FailedPutCount > 0) {
                console.error(`[Firehose Destination] FailedPutCount ${resp.FailedPutCount}`);

                // retry after delay:
                delay(5 * SECONDS)
                  .then(() => {
                    sendAttempt(records.slice(-1 * Number(resp.FailedPutCount)));
                  });
                return;
              }

              // job finished:
              console.log(`[Firehose Destination] finished sending ${params.Records.length} messages`);
              resolve(resp);
            });
          };
          sendAttempt(records);
      ...
      splitIntoBatches(params.Records, (err, chunks) => {
        if (err) {
          console.error(`[Firehose Destination] could not split records into batches`, err);
          return reject(err);
        }

        const messagesTotal = params.Records.length;
        console.log(`[Firehose Destination] pushing ${messagesTotal} messages in ${chunks.length} chunk(s)`);
        ...
        Promise
          .all(promises)
          .then(responses => {
            console.log(`[Firehose Destination] finished sending ${messagesTotal} messages in ${chunks.length} chunk(s)`);
            resolve(responses);
          })
